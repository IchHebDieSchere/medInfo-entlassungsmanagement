...
const run = async () => {
  ...
  const encounterId =
    encounterBundle.entry?.[0]?.resource?.id ?? seed.encounter.id
  ...
  const composition = await createDischargeComposition({
    ...
    sections: [
      ...
      {
        sectionTitle: 'Empfehlung',
        text: 'Wiedervorstellung bei Beschwerden, Kontrolle beim Hausarzt in 2 Wochen.'
      }
    ]
  })
